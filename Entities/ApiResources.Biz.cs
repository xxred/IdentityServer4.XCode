...
namespace IdentityServer4.XCode.Entities
{
    ...
    public partial class ApiResources : Entity<ApiResources>
    {
        ...
        public static IList<ApiResources> FindAllByScopeNames(string[] scopeNames)
        {
            var apiScopes = ApiScopes.FindAllByScopeNames(scopeNames);
            var apiResourceIds = apiScopes.Select(s => s.ApiResourceId);

            // 实体缓存
            if (Meta.Session.Count < 1000) return Meta.Cache.FindAll(e => apiResourceIds.Any(a => a == e.Id));

            return FindAll(_.Id.In(apiResourceIds));
        }
        #endregion
